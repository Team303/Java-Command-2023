    ...
    public void execute() {
        ...
        if (Robot.controllerChooser.getSelected().equals("Controller")) {
                if (Robot.isReal())
                        Robot.swerve.drive(
                                new Translation2d(
                                        DEADBAND_FILTER.applyDeadband(Robot.getOperatorXbox().getLeftY(), DEADBAND_FILTER.getLowerBound())
                                                * Swerve.MAX_VELOCITY * MAX_DRIVE_SPEED,
                                        DEADBAND_FILTER.applyDeadband(Robot.getOperatorXbox().getLeftX(), DEADBAND_FILTER.getLowerBound())
                                                * Swerve.MAX_VELOCITY * MAX_DRIVE_SPEED),
                                DEADBAND_FILTER.applyDeadband(-Robot.getOperatorXbox().getRightX(), DEADBAND_FILTER.getLowerBound()) * 4,
                                fieldOriented);
                else 
                        Robot.swerve.drive(
                                new Translation2d(
